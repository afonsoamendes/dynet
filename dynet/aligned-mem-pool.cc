...
AlignedMemoryPool::AlignedMemoryPool(const std::string &name,
                                     ...
                                     bool dynamic)
    : name(name), cap(initial_cap), current(0), a(a),
    expanding_unit(expanding_unit), dynamic(dynamic) {
  DYNET_ARG_CHECK(cap > 0, "Attempt to allocate memory of size 0 in AlignedMemoryPool");
  if (dynamic) {
    pools.push_back(new DynamicCPUMemoryPool(name, cap, a));
  } else {
...
void* AlignedMemoryPool::allocate(size_t n) {
  ...
  if (res == 0) {
    ...
    if (dynamic) {
      pools.push_back(new DynamicCPUMemoryPool(name, new_pool_size, a));
    } else {
