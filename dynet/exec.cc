...
const Tensor& SimpleExecutionEngine::incremental_forward(VariableIndex i) {
  ...
  if (i >= num_nodes_evaluated) {
    ...
    for (unsigned j = num_nodes_evaluated; j <= i; ++j) {
      ...
      auto rounded_n = pool_fxs->a->round_up_align(node->dim.size() * sizeof(float));
      size += rounded_n;
    }
...
void SimpleExecutionEngine::backward(VariableIndex from_where, bool full) {
  ...
  // TODO(oir): This assumes all of these yse tge same device!!
  unsigned size = 0;
  void* begin;
  for (unsigned i = 0; i < num_nodes; ++i) {
    const Node* node = cg.nodes[i];
    auto rounded_n = pool_dEdfs->a->round_up_align(node->dim.size() * sizeof(float));
    size += rounded_n;
  }
  ...
  for (unsigned i = 0; i < num_nodes; ++i) {
    ...
    auto rounded_n = pool_dEdfs->a->round_up_align(dim.size() * sizeof(float));
    begin += rounded_n;
